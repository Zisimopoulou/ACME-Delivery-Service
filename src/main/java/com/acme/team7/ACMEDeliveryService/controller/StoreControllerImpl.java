...
public class StoreControllerImpl extends BaseControllerImpl<Store> {
    ...
    public ResponseEntity<ApiResponse<List<KeyThreeValues<String,String,String,String>>>> reportTop10StoreProducts() {
        return ResponseEntity.ok(ApiResponse.<List<KeyThreeValues<String,String,String,String>>>builder().data(storeService.reportTop10StoreProducts()).build());
    }
    ...
    public ResponseEntity<ApiResponse<List<KeyTwoValues<String,String,String>>>> reportTopStoresPerCategory(@PathVariable("id") final Long id) {
        List<KeyTwoValues<String,String,String>> stores = storeService.reportTopStoresPerCategory(id);
        if (stores.isEmpty()) {
            throw new NoSuchElementException("Unable to find a stores.");
        }
